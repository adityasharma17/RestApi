...
public class MainActivity extends Activity implements View.OnClickListener, SSHView.OnInputTextListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        Bundle extras = getIntent().getExtras();
         floatIp = extras.getString("floatIp");
         username = extras.getString("username");
         password1 = extras.getString("password");
        setContentView(R.layout.activity_mainssh);
        initView();
        initData();
        setLisetener();
        input();
...
//      ll_ssh_keyboard = (LinearLayout)findViewById(R.id.ll_ssh_keyboard);
    ...
    public void onClick(View v) {
       // switch (v.getId()) {
        ...
        if(v.getId() == R.id.ssh_key_tab)
        { keyListener.sendTab();}
        if(v.getId() == R.id.ssh_key_left)
        {
            keyListener.sendPressedKey(vt320.KEY_LEFT);
        }
        if(v.getId() == R.id.ssh_key_right)
        {
            keyListener.sendPressedKey(vt320.KEY_RIGHT);
        }
        if(v.getId() == R.id.ssh_key_up)
        {
            keyListener.sendPressedKey(vt320.KEY_UP);
        }
        if(v.getId() == R.id.ssh_key_down)
        {
            keyListener.sendPressedKey(vt320.KEY_DOWN);
        }
        if(v.getId() == R.id.ssh_key_sp_1)
